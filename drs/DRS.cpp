/********************************************************************
  ...
  $Id: DRS.cpp 21215 2013-12-19 13:28:40Z ritt $
...
int DRSBoard::GetTime(unsigned int chipIndex, int channelIndex, int tc, float *time, bool tcalibrated, bool rotated)
{
   ...
   if (fDRSType < 4)
      return GetTime(chipIndex, fNominalFrequency, tc, time, tcalibrated, rotated);
   ...
   for (i=1 ; i<fChannelDepth ; i++) {
      if (rotated)
         time[i] = time[i-1] + fCellDT[chipIndex][channelIndex][(i-1+tc) % kNumberOfBins];
      else
         time[i] = time[i-1] + fCellDT[chipIndex][channelIndex][(i-1) % kNumberOfADCBins];
   }
...
int DRSBoard::GetTimeCalibration(unsigned int chipIndex, int channelIndex, int mode, float *time, bool force)
{
   ...
   if (mode == 0) {
      ...
      for (i=0,tint=0; i<kNumberOfBins ; i++) {
         ...
         tint += fCellDT[chipIndex][channelIndex][i];
      }
   }
                               ...
                               int prog1, int prog2, unsigned short *awf, int n, bool rotated)
{
   ...
   */
   ...
   for (i=0 ; i<nChip ; i++)
      for (j=0 ; j<nChan ; j++)
         for (k=0 ; k<kNumberOfBins ; k++)
            awf[(i*nChan+j)*kNumberOfBins+k] = ave->RobustAverage(100, i, j, k);
...
{
   int    index, status, tCell, i, j, c, chip, mode, nIterPeriod, nIterSlope, clkon, phase, refclk, trg1, trg2, n_error, channel;
